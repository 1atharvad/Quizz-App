...
$("#deleteall-resources").on("click", function() {
    $.ajax({
        type: "GET",
        url: "http://localhost:3000/Harry-Potter",
        success: function(result) {
            deleteCycle(1, result[result.length-1].id);
        }
    });
});
...
function getdata() {
    $.ajax({
        ...
        success: function(result) {
            if (result.length !== 0) {
                deleteCycle(result[0].id, result[result.length-1].id);
            }
        }
    });
}

function deleteCycle(cnt, size) {
    console.log(cnt, size)
    if ((cnt > size) === false) {
        $.ajax({
            method:'DELETE',
            url: "http://localhost:3000/Harry-Potter/" + cnt,
            success: function() {
            }
        }).then(function() {
            console.log(cnt + " Deleted"); 
            deleteCycle(cnt+1, size);
        });
    } else {
        getdata();
    }
